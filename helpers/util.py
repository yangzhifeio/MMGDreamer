...
from model.diff_utils.util_3d import sdf_to_mesh, sdf_to_mesh_texture
...
def get_database_objects(boxes, datasize, cat_ids, classes, mesh_dir, render_boxes=False, colors=None, without_lamp=False):
    ...
    bbox_file = "/data/yangzhifei/project/MMGDreamer/FRONT/cat_jid_trainval.json" if datasize == 'large' else "/data/yangzhifei/project/MMGDreamer/FRONT/cat_jid_trainval_small.json"
    colors = iter(colors)
    ...
    trimesh_meshes = []
    raw_meshes = []
    model_base_path = "/data/yangzhifei/project/MMGDreamer/FRONT/3D-FUTURE-model"
    instance_id = 1
...
def pytorch3d_to_trimesh_texture(pytorch3d_mesh):
    trimesh_verts = pytorch3d_mesh.verts_list()[0].cpu().numpy()
    trimesh_faces = pytorch3d_mesh.faces_list()[0].cpu().numpy()
    # trimesh_normals = pytorch3d_mesh.verts_normals_list()[0].cpu().numpy()
    tri_mesh = trimesh.Trimesh(vertices=trimesh_verts, faces=trimesh_faces, process=False)
...
def get_generated_shapes(boxes, shapes, cat_ids, classes, mesh_dir, render_boxes=False, colors=None, without_lamp=False):
    ...
    for j in range(0, boxes.shape[0]):
        ...
        obj = next(trimesh_meshes)
        color = next(colors)
        obj.visual.vertex_colors = color  # 设置颜色
        obj.visual.face_colors = color
        raw_obj_list.append(obj.copy())
        obj.export(os.path.join(mesh_dir, query_label + '_' + str(cat_ids[j]) + "_" + str(instance_id)+".obj"))
        instance_id += 1
...
def get_generated_texture(boxes, shapes, cat_ids, classes, mesh_dir, render_boxes=False, colors=None, without_lamp=False):
    mesh_gen = sdf_to_mesh_texture(shapes,render_all=True)
    colors = iter(colors)
    trimesh_meshes = iter([pytorch3d_to_trimesh_texture(mesh) for mesh in mesh_gen])
    obj_list = []
    lamp_mesh_list = []
    raw_obj_list = []
    instance_id = 1
