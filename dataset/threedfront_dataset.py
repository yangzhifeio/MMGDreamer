...
class ThreedFrontDatasetSceneGraph(data.Dataset):
    def __init__(self, root, root_3dfront='', split='train', shuffle_objs=False, pass_scan_id=False, use_SDF=False,
                 ...
                 eval_type='addition', with_CLIP=False, use_image=False, bin_angle=False,
                 seed=True, large=False, recompute_feats=False, recompute_clip=False, use_image_scene_rels=False,
                 room_type='bedroom'):
        ...
        self.use_image_scene_rels = use_image_scene_rels
        ...
        self.relationship_json, self.objs_json, self.tight_boxes_json, self.images_json = \
        ...
        for scene, infos in self.tight_boxes_json.items():
            for id, info in infos.items():
                if 'model_path' in info:
                    if info['model_path']:
                        info['model_path'] = root + info['model_path'][39:]  # You need to make changes based on your path
        ...
        if self.with_CLIP:
            self.cond_model, self.preprocess = clip.load("ViT-B/32", device='cuda')
            self.cond_model_cpu, self.preprocess_cpu = clip.load("ViT-B/32", device='cpu')
            print('loading CLIP')
            print('Checking for missing clip feats. This can be slow the first time.')
            # for index in tqdm(range(len(self))):
            #     self.__getitem__(index)
            # if you first to run this program, you need to release this 
            ...
            for scan in data['scans']:
                ...
                for k, v in scan["objects"].items():
                    ...
                    try:
                        ...
                        print(e)
                    try:
                        model_path = box_data[scan["scan"]][k]["model_path"]
                        boxes[int(k)]['model_path']  = model_path
                        if self.use_image and model_path is not None:
                            image[int(k)] = {}
                            image[int(k)]['image_path'] = box_data[scan["scan"]][k]["model_path"].replace("raw_model.obj", "image.jpg")
                    except Exception as e:
                        print(e)
                        continue
                ...
                images[scan["scan"]] = image
        return rel, objs, tight_boxes, images
    ...
    def __getitem__(self, index):
        ...
        # instance2label, the whole instance ids in this scene e.g. {1: 'wardrobe', 2: 'double_bed', 3: 'table', 4: 'cabinet', 5: 'nightstand', 6: 'ceiling_lamp', 7: 'floor'}
        instance2label = self.objs_json[scan_id]
        keys = list(instance2label.keys())
        instance2imagepath = self.images_json[scan_id]
        ...
        if self.use_image:
            self.clip_image_feats_path = os.path.join(self.root_3dfront, scan_id,
                                                'CLIP_images{}.pkl'.format(scan_id))
            if not self.large:
                self.clip_image_feats_path = os.path.join(self.root_3dfront, scan_id,
                                                        'CLIP_image_small_{}.pkl'.format(scan_id))
            if not os.path.exists(os.path.join(self.root_3dfront, scan_id)):
                os.makedirs(os.path.join(self.root_3dfront, scan_id))
            if self.recompute_clip:
                self.clip_image_feats_path += 'tmp'
        ...
        for key in keys:
            ...
            if not self.large:
                scene_class_id_grained = self.fine_grained_classes[scene_instance_class]  # ID in all instances
                scene_instance_class = self.mapping_full2simple[scene_instance_class]   # kids_bed -> bed
                scene_class_id = self.classes[scene_instance_class]  # simple class ID
            else:
        ...
        for r in rel_json: # create relationship triplets from data
            if r[0] in instance2mask.keys() and r[1] in instance2mask.keys():
                subject = instance2mask[r[0]] - 1  # subject - 1
        ...
        if self.use_scene_rels:
            ...
            if self.use_SDF:
                obj_sdf_list.append(torch.zeros((1, self.sdf_res, self.sdf_res, self.sdf_res))) # _scene_  attention it
        ...
        if self.with_CLIP:
            ...
            if os.path.exists(self.clip_feats_path):
                ...
                clip_feats_ins = clip_feats_dic['instance_feats']  # [instance_num, 512]
                clip_feats_order = np.asarray(clip_feats_dic['instance_order'])  # [instance_num]
                ordered_feats = []
                if len(clip_feats_ins) - len(clip_feats_order) == 1:  # there is a _scene_ inside the file
                    for inst in instances_order:
                        clip_feats_in_instance = inst == clip_feats_order  # inst == clip_feats_order, return bool of list
                        ordered_feats.append(clip_feats_ins[:-1][clip_feats_in_instance])   # hope to not include _scene_ feature
                else:
                    for inst in instances_order:
        ...
        # 2024.5.27  yzf add
        if self.use_image:
            if os.path.exists(self.clip_image_feats_path):
                clip_image_feats_dic = pickle.load(open(self.clip_image_feats_path, 'rb'))
                clip_image_feats_ins = clip_image_feats_dic['instance_image_feats']
                clip_image_feats_order = np.asarray(clip_feats_dic['instance_order'])  # [instance_num]
                ordered_image_feats = []
                if len(clip_image_feats_ins) - len(clip_image_feats_order) == 1:  # there is a _scene_ inside the file
                    for inst in instances_order:
                        clip_image_feats_in_instance = inst == clip_image_feats_order
                        ordered_image_feats.append(clip_image_feats_ins[:-1][clip_image_feats_in_instance])   # hope to not include _scene_ feature
                else:
                    for inst in instances_order:
                        clip_image_feats_in_instance = inst == clip_image_feats_order
                        ordered_image_feats.append(clip_image_feats_ins[clip_image_feats_in_instance])
                if self.use_image_scene_rels:
                    ordered_image_feats.append(clip_image_feats_ins[-1][np.newaxis,:]) # should be room's feature
                clip_image_feats_ins = list(np.concatenate(ordered_image_feats, axis=0))
        ...
        if self.with_CLIP and (not os.path.exists(self.clip_feats_path) or clip_feats_ins is None) and self.cond_model is not None:
            ...
            with torch.no_grad():
                ...
                if self.use_scene_rels:
                    obj_cat.pop()
                    obj_cat.append('room') # TODO check, replace _scene_ to "room"
        ...
        if self.use_image and (not os.path.exists(self.clip_image_feats_path) or clip_image_feats_ins is None) and (self.cond_model is not None and self.preprocess is not None):
            obj_image_cat = []
            with torch.no_grad():
                for key in instance2imagepath:
                    image_fea = self.preprocess(Image.open(instance2imagepath[key]["image_path"])).to("cuda")
                    obj_image_cat.append(image_fea)
                obj_image_cat.append(torch.zeros([3, 224, 224]).float().to("cuda"))  # add floor feature
                if self.use_image_scene_rels:
                    obj_image_cat.append(torch.zeros([3, 224, 224]).float().to("cuda"))  # last two is floor and scene node image feature
                obj_image_cat = torch.stack(obj_image_cat, dim=0)
                image_obj_feature = self.cond_model.encode_image(obj_image_cat).detach().cpu().numpy()
            
            clip_image_feats_in = {}
            clip_image_feats_in['instance_image_feats'] = image_obj_feature
            clip_image_feats_in['instance_order'] = instances_order # this doesn't include room (_scene_)
            path = os.path.join(self.clip_image_feats_path)
            if self.recompute_clip:
                path = path[:-3]
            pickle.dump(clip_image_feats_in, open(path, 'wb'))
            clip_image_feats_ins = list(clip_image_feats_in['instance_image_feats'])
        ...
        if self.use_image:
            output['encoder']['image_feats'] = clip_image_feats_ins
        ...
        if not self.with_changes:
            ...
            if not self.eval:
                if self.with_changes:
                    ...
                    output['manipulate']['type'] = 'none'
                output['decoder'] = copy.deepcopy(output['encoder'])  # attention it 
                if output['manipulate']['type'] == 'addition':
                    node_id, node_removed, node_clip_removed, node_image_clip_removed, triples_removed, triples_clip_removed, words_removed = self.remove_node_and_relationship(output['encoder'])
                    if node_id >= 0:
                        ...
                        output['manipulate']['added_node_images_clip'] = node_image_clip_removed
                    else:
                        output['manipulate']['type'] = 'none'
        ...
        if self.use_image:
            output['encoder']['image_feats'] = torch.from_numpy(np.array(output['encoder']['image_feats'], dtype=np.float32))
        ...
        if self.use_image:
            output['decoder']['image_feats'] = torch.from_numpy(np.array(output['decoder']['image_feats'], dtype=np.float32))
    ...
    def remove_node_and_relationship(self, graph):
        ...
        excluded = [self.classes['floor']]  # first node id is scene, the last one is floor
        ...
        node_clip_image_removed = None
        if self.with_CLIP:
            node_clip_removed = graph['text_feats'].pop(node_id)
        if self.use_image:
            node_clip_image_removed = graph["image_feats"].pop(node_id)
        ...
        return node_id, node_removed, node_clip_removed, node_clip_image_removed, triples_removed, triples_clip_removed_list, words_removed_list
    ...
    def modify_relship(self, graph, interpretable=False):
        ...
        while not did_change and trials < 1000:
            ...
            if graph['objs'][obj] in excluded or graph['objs'][sub] in excluded:
                continue
            if interpretable:   # inference adapt it, often reverse relationship
        ...
        # idx: idx-th triple; (sub, pred, obj): previous triple, modify encoder relationship
    ...
    def collate_fn(self, batch):
        ...
        for i in range(len(batch)):
            ...
            global_node_id += len(batch[i]['encoder']['objs'])  # save one batch of data, glo node id is the number of nodes in this batch
            global_dec_id += len(batch[i]['decoder']['objs'])  # save one batch of data, glo dec id is the number of nodes in this batch
        ...
        for key in ['encoder', 'decoder']:
            ...
            for i in range(len(batch)):
                ...
                if 'image_feats' in batch[i][key]:
                    all_image_feats.append(batch[i][key]['image_feats'])
            ...
            if len(all_image_feats) > 0:
                all_image_feats = torch.cat(all_image_feats)
                outputs['image_feats'] = all_image_feats
...
if __name__ == "__main__":
    dataset = ThreedFrontDatasetSceneGraph(
        root="/data/yangzhifei/project/echoscene/FRONT",
        split='val_scans',
        shuffle_objs=True,
        use_SDF=False,
        use_scene_rels=True,
