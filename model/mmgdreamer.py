...
from model.networks.diffusion_shape.mmg2shape import MMGToShape
...
from model.networks.diffusion_layout.mmg2layout import MMGToLayout
import numpy as np
...
class Sg2ScDiffModel(nn.Module):
    def __init__(self, vocab, diff_opt, diffusion_bs=8, embedding_dim=128, batch_size=32,
        ...
        self.out_dim_ini_encoder = gconv_dim * 2 + add_dim + add_image_dim
        gconv_kwargs_ec = {
            'input_dim_obj': gconv_dim * 2 + add_dim + add_image_dim,
        ...
        self.out_dim_manipulator = gconv_dim * 2 + add_dim + add_image_dim
        gconv_kwargs_manipulation = {
            'input_dim_obj': self.out_dim_ini_encoder + gconv_dim + gconv_dim * 2 + add_dim + add_image_dim, # latent_f + change_flag + obj_embedding + clip
            'input_dim_pred': gconv_dim * 2 + add_dim,
            'hidden_dim': gconv_hidden_dim,
            'pooling': gconv_pooling,
        ...
        if self.s_l_separated:
            gconv_kwargs_ec_rel = {
                'input_dim_obj': self.out_dim_manipulator + gconv_dim * 2 + add_dim + add_image_dim,
                'input_dim_pred': gconv_dim * 2 + add_dim,
                'hidden_dim': gconv_hidden_dim,
                'pooling': gconv_pooling,
                'num_layers': gconv_num_layers,
        ...
        ## shape branch
        self.ShapeDiff = MMGToShape(self.diff_cfg)
        self.visualizer = Visualizer(self.diff_cfg) # visualizer
        if get_rank() == 0:
            self.visualizer.setup_io()
        rel_s_layers = [gconv_dim * 2 + add_dim + add_image_dim, 960, 1280] # cross attn
        if self.ShapeDiff.df.conditioning_key == 'concat':
            rel_s_layers = [gconv_dim * 2 + add_dim + add_image_dim, 1280, 4096]
        ...
        ## layout branch
        self.LayoutDiff = MMGToLayout(self.diff_cfg)
    ...
    def init_encoder(self, objs, triples, enc_text_feat, enc_rel_feat, enc_image_feat):
        O, T = objs.size(0), triples.size(0)
        s, p, o = triples.chunk(3, dim=1)  # All have shape (T, 1)
        s, p, o = [x.squeeze(1) for x in [s, p, o]]  # Now have shape (T,)
        edges = torch.stack([s, o], dim=1)  # Shape is (T, 2)

        obj_embed = self.obj_embeddings_ec(objs)  # [B, 128]
        pred_embed = self.pred_embeddings_ec(p)

        if self.clip and not self.use_image:
            obj_embed = torch.cat([enc_text_feat, obj_embed], dim=1)
            pred_embed = torch.cat([enc_rel_feat, pred_embed], dim=1)
        elif self.clip and self.use_image:
            obj_embed = torch.cat([enc_text_feat, enc_image_feat, obj_embed], dim=1)
            pred_embed = torch.cat([enc_rel_feat, pred_embed], dim=1)
    ...
    def manipulate(self, latent_f, objs, triples, dec_text_feat, dec_rel_feat, dec_image_feat, mask_text_objs=None, mask_rel_triples=None):
        s, p, o = triples.chunk(3, dim=1)  # All have shape (T, 1), come from decoder
        s, p, o = [x.squeeze(1) for x in [s, p, o]]  # Now have shape (T,)
        edges = torch.stack([s, o], dim=1)  # Shape is (T, 2)
        ...
        # zero mask
        obj_embed[mask_text_objs] = 0
        # pred_embed[mask_rel_triples] = 0
        if self.clip and not self.use_image:
            obj_embed = torch.cat([dec_text_feat, obj_embed], dim=1)
            pred_embed = torch.cat([dec_rel_feat, pred_embed], dim=1)
        elif self.clip and self.use_image:
            obj_embed = torch.cat([dec_text_feat, dec_image_feat, obj_embed], dim=1)
            pred_embed = torch.cat([dec_rel_feat, pred_embed], dim=1)

        obj_vecs_ = torch.cat([latent_f, obj_embed], dim=1)  # is same as Zi of the paper.
    ...
    def forward(self, enc_objs, enc_triples, enc_text_feat, enc_rel_feat, enc_image_feat, dec_objs, dec_objs_grained,
                dec_triples, dec_boxes, dec_text_feat, dec_rel_feat, dec_image_feat, dec_objs_to_scene, missing_nodes, 
                manipulated_nodes, dec_sdfs, dec_angles, epoch, mask_text_objs, mask_rel_triples):

        obj_embed, _, latent_obj_vecs, latent_pred_vecs = self.init_encoder(enc_objs, enc_triples, enc_text_feat, enc_rel_feat, enc_image_feat)

        # append zero nodes, if add node
        nodes_added = []
        ...
        # normal [latent_obj_vecs, zeros], nodes_added  [zeros, random.normal]
        change_repr = []
        ...
        latent_obj_vecs_, pred_vecs_ , obj_embed_, pred_embed_ = self.manipulate(latent_obj_vecs_, dec_objs, dec_triples, dec_text_feat, dec_rel_feat, dec_image_feat, mask_text_objs, mask_rel_triples) # contains all obj now
        ...
        if not self.replace_all_latent:
            ...
            latent_obj_vecs = latent_obj_vecs_  # going there

        # relation embeddings -> diffusion
        # c_rel_feat_s under thorught manipulate network, uc_rel_feat_s is origin embedding + CLIP from decoder
        c_rel_feat_s = latent_obj_vecs   # manipulate 
        uc_rel_feat_s = self.rel_s_mlp(obj_embed_) # embedding + CLIP from decoder
        ...
        Shape_loss, Shape_loss_dict = self.ShapeDiff.forward(epoch)
    ...
    def manipulate_ori(self, latent_f, objs, triples, dec_text_feat, dec_rel_feat, dec_image_feat):
        s, p, o = triples.chunk(3, dim=1)  # All have shape (T, 1), come from decoder
        s, p, o = [x.squeeze(1) for x in [s, p, o]]  # Now have shape (T,)
        edges = torch.stack([s, o], dim=1)  # Shape is (T, 2)
    ...
    def manipulate_mask(self, latent_f, objs, triples, dec_text_feat, dec_rel_feat, dec_image_feat, mask_type):
        s, p, o = triples.chunk(3, dim=1)  # All have shape (T, 1), come from decoder
        s, p, o = [x.squeeze(1) for x in [s, p, o]]  # Now have shape (T,)
        edges = torch.stack([s, o], dim=1)  # Shape is (T, 2)
        ...
        # obj_embed[mask_text_objs] = 0
        if mask_type == "one" or mask_type == "three":
            pred_embed = torch.zeros_like(pred_embed)   # mask edge feature
            dec_rel_feat = torch.zeros_like(dec_rel_feat)
        if mask_type == "three" or mask_type == "five":  # mask text feature
            obj_embed = torch.zeros_like(obj_embed)
            dec_text_feat = torch.zeros_like(dec_text_feat)
    ...
    def sample(self, dec_objs, dec_triplets, dec_text_feat, dec_rel_feat, dec_image_feat, gen_shape=False):
        with torch.no_grad():
            ...
            #                                                                         dec_rel_feat, dec_image_feat)  # normal message passing  yzf 2024.6.25 modify the file
            latent_obj_vecs_, pred_vecs_, obj_embed_, pred_embed_ = self.manipulate_ori(latent_obj_vecs_, dec_objs,
                                                                                    dec_triplets, dec_text_feat,
                                                                                    dec_rel_feat, dec_image_feat)  # normal message passing
    ...
    def sample_with_changes(self, enc_objs, enc_triples, enc_text_feat, enc_rel_feat, enc_image_feat, dec_objs, dec_triplets, dec_text_feat, dec_rel_feat, dec_image_feat, manipulated_nodes, gen_shape=False):
        with torch.no_grad():
    ...
    def sample_with_additions(self, enc_objs, enc_triples, enc_text_feat, enc_rel_feat, enc_image_feat, dec_objs, dec_triplets, dec_text_feat, dec_rel_feat, dec_image_feat, missing_nodes, gen_shape=False):
        with torch.no_grad():
            obj_embed, pred_embed, latent_obj_vecs, latent_pred_vecs = self.init_encoder(enc_objs, enc_triples, enc_text_feat, enc_rel_feat, enc_image_feat)
            # append zero nodes, missing_nodes is manipulated added node
            nodes_added=[]
            for i in range(len(missing_nodes)):
                ad_id = missing_nodes[i] + i
                nodes_added.append(ad_id)
            ...
            change_repr = torch.stack(change_repr, dim=0)
            latent_obj_vecs_ = torch.cat([latent_obj_vecs, change_repr], dim=1)
            latent_obj_vecs_, pred_vecs_, obj_embed_, pred_embed_ = self.manipulate(latent_obj_vecs_, dec_objs, dec_triplets, dec_text_feat, dec_rel_feat, dec_image_feat)
    ...
    def init_encoder_oper(self, objs, triples, enc_text_feat, enc_rel_feat, enc_image_feat, mask_text_objs_enc):
        O, T = objs.size(0), triples.size(0)
        s, p, o = triples.chunk(3, dim=1)  # All have shape (T, 1)
        s, p, o = [x.squeeze(1) for x in [s, p, o]]  # Now have shape (T,)
        edges = torch.stack([s, o], dim=1)  # Shape is (T, 2)
        ...
        pred_embed = self.pred_embeddings_ec(p)
        # mask
        obj_embed[mask_text_objs_enc] = 0
    ...
    def init_encoder_three(self, objs, triples, enc_text_feat, enc_rel_feat, enc_image_feat):
        O, T = objs.size(0), triples.size(0)
        s, p, o = triples.chunk(3, dim=1)  # All have shape (T, 1)
        s, p, o = [x.squeeze(1) for x in [s, p, o]]  # Now have shape (T,)
        edges = torch.stack([s, o], dim=1)  # Shape is (T, 2)
        ...
        # mask
        obj_embed = torch.zeros_like(obj_embed)
        enc_text_feat = torch.zeros_like(enc_text_feat)
        ...
        return obj_embed, pred_embed, latent_obj_f, latent_pred_f
    def sample_with_changes_mask(self, enc_objs, enc_triples, enc_text_feat, enc_rel_feat, enc_image_feat, dec_objs, dec_triplets, dec_text_feat, dec_rel_feat, dec_image_feat, manipulated_nodes, gen_shape=False, mask_text_objs_enc=None, mask_text_objs=None):
        with torch.no_grad():
            obj_embed, pred_embed, latent_obj_vecs, latent_pred_vecs = self.init_encoder_oper(enc_objs, enc_triples,
                                                                                         enc_text_feat,
                                                                                         enc_rel_feat,
                                                                                         enc_image_feat, mask_text_objs_enc)
            # mark changes in nodes
            change_repr = []
            for i in range(len(latent_obj_vecs)):
                if i not in manipulated_nodes:
            ...
            latent_obj_vecs_, pred_vecs_, obj_embed_, pred_embed_ = self.manipulate(latent_obj_vecs_, dec_objs,
                                                                                    dec_triplets, dec_text_feat,
                                                                                    dec_rel_feat, dec_image_feat, mask_text_objs)
            if not self.replace_all_latent:
                # take original nodes when untouched
                touched_nodes = torch.tensor(sorted(manipulated_nodes)).long()
                for touched_node in touched_nodes:
        ...
        keep = torch.from_numpy(np.asarray(keep).reshape(-1, 1)).float().cuda()
        return keep, {'shapes': gen_sdf}, layout_dict
    def sample_with_changes_mask_three(self, enc_objs, enc_triples, enc_text_feat, enc_rel_feat, enc_image_feat, dec_objs, dec_triplets, dec_text_feat, dec_rel_feat, dec_image_feat, manipulated_nodes, gen_shape=False, mask_type=None):
        with torch.no_grad():
            obj_embed, pred_embed, latent_obj_vecs, latent_pred_vecs = self.init_encoder_three(enc_objs, enc_triples,
                                                                                         enc_text_feat,
                                                                                         enc_rel_feat,
                                                                                         enc_image_feat)
            # mark changes in nodes
            ...
            change_repr = torch.stack(change_repr, dim=0)
            latent_obj_vecs_ = torch.cat([latent_obj_vecs, change_repr], dim=1)
            latent_obj_vecs_, pred_vecs_, obj_embed_, pred_embed_ = self.manipulate_mask(latent_obj_vecs_, dec_objs,
                                                                                    dec_triplets, dec_text_feat,
                                                                                    dec_rel_feat, dec_image_feat, mask_type)
            if not self.replace_all_latent:
                # take original nodes when untouched
                touched_nodes = torch.tensor(sorted(manipulated_nodes)).long()
                for touched_node in touched_nodes:
        ...
        keep = torch.from_numpy(np.asarray(keep).reshape(-1, 1)).float().cuda()
        return keep, {'shapes': gen_sdf}, layout_dict
    def sample_with_additions_mask(self, enc_objs, enc_triples, enc_text_feat, enc_rel_feat, enc_image_feat, dec_objs, dec_triplets, dec_text_feat, dec_rel_feat, dec_image_feat, missing_nodes, gen_shape=False, mask_text_objs_enc=None, mask_text_objs=None):
        with torch.no_grad():
            obj_embed, pred_embed, latent_obj_vecs, latent_pred_vecs = self.init_encoder_oper(enc_objs, enc_triples, enc_text_feat, enc_rel_feat, enc_image_feat, mask_text_objs_enc)
            # append zero nodes, missing_nodes is manipulated added node
            nodes_added=[]
            for i in range(len(missing_nodes)):
                ad_id = missing_nodes[i] + i
            ...
            change_repr = torch.stack(change_repr, dim=0)
            latent_obj_vecs_ = torch.cat([latent_obj_vecs, change_repr], dim=1)
            latent_obj_vecs_, pred_vecs_, obj_embed_, pred_embed_ = self.manipulate(latent_obj_vecs_, dec_objs, dec_triplets, dec_text_feat, dec_rel_feat, dec_image_feat, mask_text_objs)
    ...
    def sample_with_additions_mask_three(self, enc_objs, enc_triples, enc_text_feat, enc_rel_feat, enc_image_feat, dec_objs, dec_triplets, dec_text_feat, dec_rel_feat, dec_image_feat, missing_nodes, gen_shape=False, mask_type=None):
        with torch.no_grad():
            obj_embed, pred_embed, latent_obj_vecs, latent_pred_vecs = self.init_encoder_three(enc_objs, enc_triples, enc_text_feat, enc_rel_feat, enc_image_feat)
            # append zero nodes, missing_nodes is manipulated added node
            nodes_added=[]
            for i in range(len(missing_nodes)):
                ad_id = missing_nodes[i] + i
            ...
            change_repr = torch.stack(change_repr, dim=0)
            latent_obj_vecs_ = torch.cat([latent_obj_vecs, change_repr], dim=1)
            latent_obj_vecs_, pred_vecs_, obj_embed_, pred_embed_ = self.manipulate_mask(latent_obj_vecs_, dec_objs, dec_triplets, dec_text_feat, dec_rel_feat, dec_image_feat, mask_type)
        ...
        keep = torch.from_numpy(np.asarray(keep).reshape(-1, 1)).float().cuda()
        return keep, {'shapes': gen_sdf}, layout_dict
    def sample_mask(self, dec_objs, dec_triplets, dec_text_feat, dec_rel_feat, dec_image_feat, gen_shape=False, mask_type=None):
        with torch.no_grad():
            obj_embed, pred_embed, latent_obj_vecs, latent_pred_vecs = self.init_encoder(dec_objs, dec_triplets,
                                                                                         dec_text_feat,
                                                                                         dec_rel_feat,
            ...
            latent_obj_vecs_, pred_vecs_, obj_embed_, pred_embed_ = self.manipulate_mask(latent_obj_vecs_, dec_objs,
                                                                                    dec_triplets, dec_text_feat,
                                                                                    dec_rel_feat, dec_image_feat, mask_type)  # normal message passing
    ...
    def sample_mask_four(self, dec_objs, dec_triplets, dec_text_feat, dec_rel_feat, dec_image_feat, gen_shape=False, mask_text_objs=None, mask_rel_triples=None):
        with torch.no_grad():
            obj_embed, pred_embed, latent_obj_vecs, latent_pred_vecs = self.init_encoder(dec_objs, dec_triplets,
                                                                                         dec_text_feat,
                                                                                         dec_rel_feat,
            ...
            latent_obj_vecs_, pred_vecs_, obj_embed_, pred_embed_ = self.manipulate(latent_obj_vecs_, dec_objs,
                                                                                    dec_triplets, dec_text_feat,
                                                                                    dec_rel_feat, dec_image_feat,
                                                                                    mask_text_objs, mask_rel_triples)  # normal message passing
