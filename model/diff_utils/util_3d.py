...
def sdf_to_mesh_texture(sdf, level=0.02, color=None, render_all=False):
    # device='cuda'
    device=sdf.device
    ...
    try:
        # p3d_mesh = pytorch3d.structures.Meshes(verts, faces, textures=pytorch3d.renderer.Textures(verts_rgb=verts_rgb))
        p3d_mesh = pytorch3d.structures.Meshes(verts, faces)
    except:
        p3d_mesh = None
...
def render_meshes(mesh_renderer, meshes, level=0.02, color=None, render_imsize=256, render_all=False):
    # device='cuda'
