...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path_to_gt_mesh",
        default="/s2/yangzhifei/project/MMGDreamer/FRONT/gt_fov90_h8_obj_meshes_refine",
        help="Path to the folder containing the real renderings"
    )
    parser.add_argument(
        "--path_to_synthesized_mesh",
        default="/data/yangzhifei/project/MMGDreamer/experiments/train_all/vis/2049/mmgscene/object_meshes",
    ...
    # cat = categories[0]
    save_path = os.path.join("/".join(args.path_to_synthesized_mesh.split("/")[:-2]), args.save_name + ".txt")
        
    print(save_path)
    for cat_id in range(0, len(categories)):
        print(f"Testing {categories[cat_id]}...")

        mmgscene_pcs = collect_pcs(args.path_to_synthesized_mesh, categories[cat_id], number=5000, transform=False, visual=False)
        print(f'loaded {mmgscene_pcs.size(0)} {categories[cat_id]} from mmgscene')

        gt_pcs = collect_pcs(args.path_to_gt_mesh, categories[cat_id], number=5000, transform=False, visual=False)
        print(f'loaded {gt_pcs.size(0)} {categories[cat_id]} from gt')
        

        mmgscene_results = compute_all_metrics(mmgscene_pcs.cuda(), gt_pcs.cuda(), batch_size=50, accelerated_cd=True)
        
        with open(save_path, "a+") as file:
            result = f"v2-{categories[cat_id]}:"
            file.write(result)
            file.write("\n")
            file.write(str(mmgscene_results))
            file.write("\n")
        
        print(f"v2-{categories[cat_id]}:",mmgscene_results)
